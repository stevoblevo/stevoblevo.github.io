...
function downHas(name){
  for (const k of keys){
    const n=String(k);
    if(name==='a' && (n==='a'||n==='A'||n==='KeyA'||n==='ArrowLeft')) return true;
    if(name==='d' && (n==='d'||n==='D'||n==='KeyD'||n==='ArrowRight')) return true;
    if(name==='w' && (n==='w'||n==='W'||n==='KeyW'||n==='ArrowUp')) return true;
    if(name==='s' && (n==='s'||n==='S'||n==='KeyS'||n==='ArrowDown')) return true;
    if(name===' ' && (n===' '||n==='Space')) return true;
    if(name==='Shift' && (n==='Shift'||n==='ShiftLeft'||n==='ShiftRight')) return true;
  }
  return false;
}
...
function reset(){
  ...
  if($('line'))$('line').textContent='Collect gold signals. Arrows or WASD. Left and right turn. Up goes.';
  if($('start'))$('start').textContent='Start 45-second run \u2197';
...
document.addEventListener('keydown',e=>{
  const keep=['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','w','a','s','d','W','A','S','D',' ','Shift'];
  if(keep.includes(e.key)||['KeyW','KeyA','KeyS','KeyD','Space','ShiftLeft','ShiftRight'].includes(e.code)){
    e.preventDefault();
    keys.add(e.key);
    if(e.code)keys.add(e.code);
  }
});
document.addEventListener('keyup',e=>{keys.delete(e.key);if(e.code)keys.delete(e.code);});
document.addEventListener('visibilitychange',()=>{
  if(document.hidden&&running){paused=true;$('line').textContent='Paused while this tab is hidden.';}
  else if(running){paused=false;last=performance.now();$('line').textContent='Collect gold signals. Arrows or WASD. Left and right turn. Up goes.';}
});
