...
self.addEventListener('fetch', event => {
  ...
  if (url.origin !== BASE.origin || !url.pathname.startsWith(BASE.pathname) || url.search || !ALLOWED.has(url.href)) return;
  event.respondWith((async () => {
    ...
    try {
      ...
      return await cache.match(event.request) || (event.request.mode === 'navigate' ? await cache.match(new URL('./index.html', BASE).href) : null) || Response.error();
    }
  })());
});
